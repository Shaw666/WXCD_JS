...
{
	DataScope_Get_Channel_Data(Sample.DC5VoltReal,1);   //将电压环PID输入 写入通道
	DataScope_Get_Channel_Data(Sample.JSACVoltReal,2);     //将电压环PID输入 写入通道
	DataScope_Get_Channel_Data(Sample.JSOutVoltReal,3);     //将电压环PID输入 写入通道
	DataScope_Get_Channel_Data(Sample.JSOutCurrentReal,4);   //将电压环PID输入 写入通道
	/*******************
	串口发送数据给上位机
	********************/
